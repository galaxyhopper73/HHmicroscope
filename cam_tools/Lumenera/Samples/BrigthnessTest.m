...
b=LucamGetPropertyRange(0,cam);		% Get current Brightness Range with GetProperty function
minprop = b.Min
maxprop = b.Max
brigthnessPropDefault=LucamGetProperty(0,cam)		% Get current Brightness Setting with Get Property function.
b = brigthnessPropDefault;
b.Value = 100;
LucamSetProperty(b,cam);				% Set High Level of Brigthness to see effect on preview.
pause(pbreak);						% Give a chance to see effect on screen.
b.Value = brigthnessPropDefault.DefaultValue;
LucamSetProperty(b,cam);	% Return Brightness to initial value.
